function [Sigma,Ustar,Vstar,CostNew] = simpleMKKMupdate_svd(PH,Sigma,GradNew,CostNew,numclass,option)
...
while costmax<costmin
    [costmax,Ustar,Vstar] = costSimpleMKKM_svd(PH,stepmax,desc,SigmaNew,numclass);
...
while (stepmax-stepmin)>option.goldensearch_deltmax*(abs(deltmax)) && stepmax > eps
    ...
    [costmedr,Ur,Vr] = costSimpleMKKM_svd(PH,stepmedr,desc,SigmaNew,numclass);
    [costmedl,Ul,Vl] = costSimpleMKKM_svd(PH,stepmedl,desc,SigmaNew,numclass);     
